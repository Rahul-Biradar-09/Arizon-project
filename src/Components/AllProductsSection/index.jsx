...
class AllProductsSection extends Component {
  ...
  makeApiCall = async () => {
    ...
    const apiUrl = activeCategoryId
      ? `${baseUrl}/category/${activeCategoryId}`
      : baseUrl;
    ...
    try {
      ...
      if (response.ok) {
        ...
        const filteredData = this.filterProducts(data);
        this.setState({
  ...
  filterProducts = (products) => {
    const { activeOptionId, searchInput, activeRatingId } = this.state;
    ...
    if (activeRatingId) {
      filtered = filtered.filter(
        (product) => Math.floor(product.rating.rate) >= parseInt(activeRatingId)
      );
    }
  ...
  changeSortby = (activeOptionId) => {
    this.setState({ activeOptionId }, this.makeApiCall);
  };

  changeRating = (activeRatingId) => {
    this.setState({ activeRatingId }, this.makeApiCall);
  };

  changeCategory = (activeCategoryId) => {
    this.setState({ activeCategoryId }, this.makeApiCall);
  };
